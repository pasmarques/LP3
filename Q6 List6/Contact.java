public class Contact {
    ...
    public Contact(String name, String email, String cellPhone, int day, int month,int year){
        ...
        if(validateNome(name)&&validateCellPhone(cellPhone)&&validateEmail(email))
        {
          this.name = name;
          this.email = email;
    ...
    {
        String res; 
        res = "contato: " + getName() + " " + getCellPhone() + " " + getEmail();
        return res;
    }
    ...
    {
        if(email.contains("@hotmail.com")) //considerando isso como dominio de email aceito
        {
            return true;
        }
        return false;
    ...
    {
        final char flag[] = cellphone.toCharArray();
